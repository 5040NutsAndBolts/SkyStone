...
//returns the point that pure pursuit will move to by looking at the line segments and finding the intersections of a circle centered at the robot with radius of the look ahead distance
    ...
    public double[] goToPosition(Point goalPoint,ArrayList<WayPoint> p,int iRel)
    {
        ...
        double relativeAngleToPoint = absoluteAngleToTarget - (MathFunctions.angleWrap(robot.prevHeading-Math.PI/2));
        ...
        relativeAngleToPoint = absoluteAngleToTarget - (MathFunctions.angleWrap(robot.prevHeading));

        double turnPowerToGoal=0;
        if(iRel<p.size()-2) {
            try {
                double absoluteAngleToSegment = Math.atan2(p.get(iRel).y-robot.y,p.get(iRel).x-robot.x);
                double relativeAngleToSegment = absoluteAngleToSegment - (MathFunctions.angleWrap(robot.prevHeading));
                ...
                }else
                {
                    ...
                    double turnPowerToNextPoint = (nextAngle  - robot.prevHeading) / Math.sqrt(Math.abs(nextAngle  - robot.prevHeading));
        ...
        return new double[]{movementXPower,movementYPower,movementTurn};
    ...
    public double[] followPath(ArrayList<WayPoint> p,double lookAheadDistance)
    {
        ...
        return goToPosition(g,p,(int) d[2]);
    }
